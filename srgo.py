...
psnr = 1  # peak signal to noise ratio
...
def sensi_curve(b):
    ...
    # = (simps(signal_sensi**2.0, x=t_sensi) /
    ...
    print(d/pc, f_mean, h_rms)
    return(f_mean, h_rms)


if flag_sensi == "y":
    M1_vals = np.linspace(3.0, 5000., 2)
    ra_vals = np.linspace(-45.0, 90.0, 4)
    dec_vals = np.linspace(-45.0, 90.0, 4)
    psi_eq_vals = np.linspace(-45.0, 90.0, 4)
    ...
    j1 = int(np.where(np.abs(fq-10**(2.0)) == np.min(np.abs(fq-10**(2.0))))[0])
