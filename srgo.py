...
if flag_range == "y":
    ...
    d_array = concatenate((np.linspace(
        10*pc, d_array1[-1], l), array([pc*10**4])), axis=None)
...
if flag_mcmc == "y" and flag_sensi != "y" and flag_err != "y" and flag_range != "y":
    ...
    with MCMC:
        ...
        M1_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "GW source mass #1 posterior [$M_{\odot}$]", lower=0.1*M1, upper=10*M1
        )
        M2_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "GW source mass #2 posterior [$M_{\odot}$]", lower=0.1*M2, upper=10*M2
        )
        r_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "Initial binary separation posterior [$au$]", lower=0.5*r, upper=2.0*r
        )
        ...
        Z_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "GW source redshift posterior", lower=0.0, upper=0.1
        )
