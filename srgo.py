...
flag_range = "n"  # find observational range of SRGO ([y]/n)
flag_srcpos_skyloc = "n"  # plot sky-loc precision vs. src position ([y]/n)
flag_psnr = "n"  # plot sky-localization area vs. PSNR ([y]/n)
...
def f(t, f0, k, Z):
    ...
    if flag_mcmc == "y" and flag_sensi != "y" and flag_err != "y" and flag_range != "y" and flag_srcpos_skyloc != "y" and flag_psnr != "y" and str(type(t)) == "<class 'numpy.ndarray'>":
        return (array([
...
def time(n, t_obs, t_ins, f0, k, Z):
    ...
    if flag_mcmc == "y" and flag_sensi != "y" and flag_err != "y" and flag_range != "y" and flag_srcpos_skyloc != "y" and flag_psnr != "y":
    ...
    if flag_mcmc == "y" and flag_sensi != "y" and flag_err != "y" and flag_range != "y" and flag_srcpos_skyloc != "y" and flag_psnr != "y" and len(t) > n:
        # to be able to use FFT trick for computing speed
        pts = len(t) - n
        t = t[:-pts]
    return t, T_timing
...
if flag_range == "y":
    ...
    d_array = array([10*pc, 10**2*pc, 266*pc, 267*pc, 10**3*pc, 1579*pc, 1580*pc, 7665*pc, 7665.0348*pc, 10**4*pc, 10**5*pc, 777847*pc, 777847.7361*pc,
    ...
    axes3.axvline(x=10, linewidth=1.0, linestyle='dashdot', color="darkorchid",
                  label="nearest ~$0.5M_{\odot}$ WD binaries")
    axes3.axvline(x=267, linewidth=1.0, linestyle='dashdot', color="hotpink",
                  label="nearest ~$1M_{\odot}$ NS binaries")
    axes3.axvline(x=1580, linewidth=1.0, linestyle='dashdot', color="deepskyblue",
                  label="nearest ~$10M_{\odot}$ BH binaries")
    axes3.axvline(x=7665.0348, linewidth=1.0, linestyle='dashdot', color="mediumseagreen",
    ...
    axes3.set_ylabel(
        "Max. effective noise ($\sigma_{eff-max}$) for PSNR=1 [s]")
    ...
    leg = axes3.legend(loc='upper right', bbox_to_anchor=(
        0.953, 1.0), facecolor=facecolour, framealpha=1, prop={'size': 5.75})
...
def sensi_curve(b):
    ...
    val_lhs = (simps(signal_sensi**2.0, x=t_sensi) /
               t_sensi[-1])**0.5  # RMS signal
    val_rhs = sigma_noise / \
        ((1.0/T_sample)*t_sensi[-1])**0.5  # effective noise
    # global d
    d = b*val_lhs/val_rhs  # value of GW source distance where val_lhs == val_rhs i.e. SNR=1
    return(f(0.0, f0_sns, k_sns, Z), abs(h_plus(0.0, inc, phase, d, f0_sns, k_sns, Z, M_sns)))


if flag_sensi == "y":
    M1_vals = np.linspace(3.0, 10.**6, 2)
    ra_vals = np.linspace(-45.0, 90.0, 4)
    dec_vals = np.linspace(-45.0, 90.0, 4)
    psi_eq_vals = np.linspace(-45.0, 90.0, 4)
    inc_vals = np.linspace(0.0, 135.0, 4)
    phase_vals = np.linspace(0.0, 315.0, 1)
    ...
    i = int(np.where(np.abs(fq-10**(-6.0)) ==
            np.min(np.abs(fq-10**(-6.0))))[0])
    ...
    plt.plot(fq, h, color='black', linewidth=1.5,
             label="LHC-GW sensitivity curve from paper-I, for $T_{obs}=3.0$ hrs")
    plt.plot(f0_array, h0_array, color='black', linestyle='dashed', linewidth=1.5,
             label="Numerically obtained sensitivity curve, for $T_{obs}=3.0$ hrs")
    plt.legend(loc=2, borderpad=0.2, labelspacing=0.2)
    plt.ylim(10**(-26.), 10**(-12.))
    plt.ylabel("GW Strain", fontweight='bold')
    plt.xlabel("GW Frequency [Hz]", fontweight='bold')
    plt.yscale('log')
    plt.xscale('log')
    plt.grid()
...
if flag_sensi != "y" and flag_err != "y" and flag_range != "y" and flag_srcpos_skyloc != "y" and flag_psnr != "y":
    ...
    #    0.0, scale=10.**(-16), size=N).astype(floatX)
        ...
        if likelihood_type == "custom":
            ...
            model = abs(sum(
                ...
                SRGO_signal(t, phim, thetam, psim, inc_prior,
                            phase_prior, d, f0, k, Z_prior, M_og)
            )) / denominator
            model = (
            ...
            model = SRGO_signal(t, phim, thetam, psim,
                                inc_prior, phase_prior, d, f0, k, Z_prior, M_og)
...
# Normalized sky-localization precision vs. GW source position sky map
if flag_srcpos_skyloc == "y":

    H = array([[6.7191, 1.6350375, 8.541225, 2.6831875, 2.2927625, 8.068375, 20.3498, 27.7887625, 6.7191, 1.6350375, 8.541225, 2.6831875, 2.2927625, 8.068375, 20.3498, 27.7887625, 6.7191],
               [68.5743875, 18.2480625, 12.7647625, 46.6767875, 6.171225, 69.1792875, 27.092825, 11.4485625, 66.0764625,
                   13.0347375, 40.1061, 8.2623875, 35.474425, 62.805625, 14.5690625, 46.5694875, 68.5743875],
               [34.6884375, 27.2294875, 7.7378125, 10.5346875, 16.87195, 1.562475, 9.0146875, 18.2820375, 4.2647625,
                   2.351, 5.2733625, 9.8365625, 22.248475, 14.8069375, 12.7907, 35.599875, 34.6884375],
               [30.6108375, 16.982575, 20.3012375, 27.8463625, 28.9728, 31.114675, 35.5154625, 18.3338875,
                   11.64687125, 15.98905, 31.5224625, 24.671925, 19.7754375, 31.581975, 10.19235, 7.7362375, 30.6108375],
               [24.18005, 29.92914286, 24.8917, 10.6359375, 15.859075, 12.367025, 8.4687125, 4.508175, 24.18005,
                   29.92914286, 24.8917, 10.6359375, 15.859075, 12.367025, 8.4687125, 4.508175, 24.18005],
               [11.64687125, 15.98905, 31.5224625, 24.671925, 19.7754375, 31.581975, 10.19235, 7.7362375, 30.6108375,
                   16.982575, 20.3012375, 27.8463625, 28.9728, 31.114675, 35.5154625, 18.3338875, 11.64687125],
               [4.2647625, 2.351, 5.2733625, 9.8365625, 22.248475, 14.8069375, 12.7907, 35.599875, 34.6884375,
                   27.2294875, 7.7378125, 10.5346875, 16.87195, 1.562475, 9.0146875, 18.2820375, 4.2647625],
               [66.0764625, 13.0347375, 40.1061, 8.2623875, 35.474425, 62.805625, 14.5690625, 46.5694875, 68.5743875,
                   18.2480625, 12.7647625, 46.6767875, 6.171225, 69.1792875, 27.092825, 11.4485625, 66.0764625],
               [6.7191, 1.6350375, 8.541225, 2.6831875, 2.2927625, 8.068375, 20.3498, 27.7887625, 6.7191,
                   1.6350375, 8.541225, 2.6831875, 2.2927625, 8.068375, 20.3498, 27.7887625, 6.7191],
               ])
    ...
    bkg = axm.pcolormesh(ras*radian, decs*radian, H,
                         cmap='cividis', alpha=1.0, zorder=99)
    figm.colorbar(bkg, orientation='vertical')
    axm.scatter(array([0.0*radian]), array([51.0*radian]), s=50.0, color='fuchsia',
                marker='x', alpha=1.0, zorder=100, label="SRGO initial position")
    axm.plot(np.linspace(-180*radian, 180*radian, 360), 51.0*radian*ones(360), color='fuchsia',
             linestyle='dotted', alpha=1.0, zorder=100, label="SRGO track due to Earth's rotation")
    axm.plot(
        [], [], ' ', label="$\sigma_{noise}=0.445$ fs \nMean PSNR = 8 \nMean sky-loc. area = $23 \pm 15$ sq. deg.")
    ...
    axm.yaxis.label.set_fontsize(12)
    axm.set_title(
        "Normalized sky-localization precision vs. GW source position")
    axm.legend(
        loc="upper right",
...
if flag_psnr == "y":
    ...
    data = [
        [0.0318, 0.0, 0.6993, 6.933,
            0.5258, 0.0036, ],
        [1.1266, 7.0033, 0.0002, 0.0933, 1.6275, 0.3993, ],
        [4.1042, 15.4982, 28.246, ],
        [177.1505, 49.4506, 44.7637, 7.7463, 91.3347, ],
        [82.0729, 21.5571, 61.1266, ],
        [49.2808, 523.9416, 107.9071, 23.9022,  41.4944, 67.4715, ],
        [505.4744, 360.3063, 507.0965, 490.3435, 200.,
            81.8696, 18.3779, 294.094, 190.7197, 66.1139, ],
        [345.1003, 804.284, 348.5287, 294.5565, 380.6765,
            317.0144, 65., 559.4345, 358.0325, 717.519],
        [128.2101, 285.523, 687.5145, 368.5221, 362.2107, 623.7359, ],
        [310.1358, 947.3397, ],
        [520.649, 1000.],
    ]

    data2 = [
        [0.3068, 8.873, 0.9853, 1.8462, 2.3012, 0.9498, ],
        [5.703, 0.5346, 0.314, 2.1047, 13.4886, 2.1634, 1.7524, 0.2213, ],
        [37.084, 7.9057, 8.8984, 80.9578, ],
        [141.8325, 45.11, ],
        [196.4728, 26.4817, 13.3204, 283.4672, ],
        [58.9919, 271.833, ],
        [242.913, 581.1466, 436.067, 475.9706, 162.7209, ],
        [602.9111, 249.0809, 603.2074, 570.7478, ],
        [400.9654, 497.6, 990.691, ],
        [718.0747, ],
        [1159.3094, 1493.8382, ],
    ]

    data3 = [
        [7.9507, 24.3463, 0.138, 3.4475, 0.7466, ],
        [0.4116, 22.1667, 8.451, 2.0855, 50.5138, 38.262, 0.0366, 4.1621, 2.1898],
        [50.2472, 11.5775, 80., 24., ],
        [47.4189, 237.746, 231.3911, 22.426, 104.4092, 63.6299, ],
        [248.8702, 190.0924, 44.7574, 119.6541, ],
        [205.3082, 176.3529, 665.675, 233.3321, 519.9005, ],
        [172.3984, 788.9095, 751.6139, 221.5351, 454.7976],
        [370.06411, 787.0069, 510.5648, ],
        [803.0741, 1327.328, 1168.7299, ],
        [],
        [1507.0556, 1852.1108, 1090.6519],
    ]
    ...
    plt.scatter(psnr, means3, marker='P',
                label="16 data points over 1 day", linewidth=1.5, color='darkmagenta')
    plt.errorbar(psnr, means3, yerr=devs3, alpha=0.5,
                 linewidth=1.5, color='darkmagenta')
    plt.scatter(psnr, means2, label="32 data points over 1 day",
                linewidth=1.5, color='orange')
    plt.errorbar(psnr, means2, yerr=devs2, alpha=0.5,
                 linewidth=1.5, color='orange')
    plt.scatter(psnr, means, marker='v',
                label="64 data points over 1 day", linewidth=1.5, color='lightseagreen')
    plt.errorbar(psnr, means, yerr=devs, alpha=0.5,
                 linewidth=1.5, color='lightseagreen')
    # plt.ylim(bottom=0.1)
    plt.xlabel("PSNR (peak signal-to-noise ratio)")
    plt.ylabel("Sky-localization Area (sq. deg.)")
    plt.minorticks_on()
