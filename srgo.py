...
N = 64  # no. of SRGO data points acquired during T_obs
psnr = 5.0  # peak signal to noise ratio
# "ligo" (Whittle likelihood in Fourier space) or "custom" likelihood functions for MCMC
likelihood_type = "ligo"
# plot evolution of orientation angles & numerical error vs. no. of data points ([y]/n)
flag_extras = "y"
flag_no_plot = "n"  # no plots generated ([y]/n)
# GW source parameters:
m1 = 1000000.0 * msol  # mass of object #1 in solar masses
m2 = 1000000.0 * msol  # mass of object #2 in solar masses
r = 0.32 * au  # initial separation between bodies in au
...
def time(n, t_obs):
    ...
    n = max(n, int(2.0*f(t_obs)*t_obs)+1)  # Nyquist–Shannon sampling theorem
    if flag_mcmc == "y":
        # to be able to use FFT trick for computing speed
        n = findPrevPowerOf2(n)
        if n < int(2.0*f(t_obs)*t_obs)+1:
            n = 2*n
...
def sensi_curve():
    """For manually calculating the SRGO sensitivity curve."""
    sigma_noise = 10**(-12)
    val_lhs = (simps(signal_plt**2.0, x=t_plt) / t_plt[-1])**0.5
    val_rhs = 2.0 * sigma_noise / ((1.0/T_sample)*t_plt[-1])**0.5
    if val_lhs == val_rhs:
        print("BINGO!, %r, %r\nf(0) = %r\nh0 = %r" %
              (val_lhs, val_rhs, f(0.0), max(abs(h_plus(t_plt, inc, phase)))))
    else:
        print("TRY d = %r\n%r, %r\nf(0) = %r\nh0 = %r" %
              (d*val_lhs/(val_rhs*10**6*pc), val_lhs, val_rhs, f(0.0), max(abs(h_plus(t_plt, inc, phase)))))
...
if flag_mcmc == "y":
    ...
    t, dt = time(N, T_obs)
    N = len(t)
    signal_func = interp1d(t_plt, signal_plt)
    signal = signal_func(t).astype(floatX)
    ...
    with MCMC:
        ...
        ra_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "GW source RA posterior [$\degree$]", lower=ra-30.0, upper=ra+30.0
        )
        dec_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=-90.0, upper=90.0)(
            "GW source DEC posterior [$\degree$]", lower=dec-30.0, upper=dec+30.0
        )
        psi_eq_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=180.0)(
            "GW polarization posterior [$\degree$]", lower=psi_eq-30.0, upper=psi_eq+30.0
        )
        phase_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "GW initial phase posterior [$\degree$]", lower=phase-30.0, upper=phase+30.0
    ...
    if __name__ == "__main__":  # needed for MCMC to use multiple CPU cores
        with MCMC:
            ...
            ax_az = az.plot_pair(
                ...
                figsize=(9.25, 9.25),
                kde_kwargs={
                    # Plot 90%, 95%, 99% HDI contours (highest density credibility regions)
                    # (x% credible region => (100 - x)% argument value)
                    "hdi_probs": [0.1],
                    "contour": True,
                    "contourf_kwargs": {"cmap": "Wistia"},
                    "fill_last": False,
                    "label": "90% HPDI contour (highest posterior density interval)",
                    "legend": True,
                },
            )
            ...
            axm.scatter(ras, decs, c=imgm, marker='.', s=0.07, linewidths=0.0,
                        cmap='Oranges', zorder=99,
                        # norm=mpl_colors.Normalize(vmin=0.0, vmax=1.0), alpha=1.0,
                        label="90%% HPDI contour\nSky-localization area = %r deg$^2$" % (
                            np.round(sky_loc_area, 5)))
...
if flag_no_plot == "n":
    ...
    f_num = 0
    for fig in figs:
        if plotname[f_num] != "joint_posterior" and plotname[f_num] != "spectrogram" and plotname[f_num] != "numerical_error":
