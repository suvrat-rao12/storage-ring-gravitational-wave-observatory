...
phase = 30.0  # GW signal initial phase in degrees
# local sidereal time of SRGO in hh:mm:ss at the start of the observation run
LST = (00.0, 00.0, 00.0)
RA = (03.0, 00.0, 00.0)  # GW source right ascension in hh:mm:ss
dec = 30.0  # GW source declination in degrees
# GW polarization angle (in degrees) as measured in equatorial celestial coordinates
psi_eq = 30.0
day = 86164.0905  # sidereal Earth day in seconds
T_obs = 1.5 * day  # duration of SRGO observation run
# input("Compute sky localization with MCMC using synthetic data? ([y]/n)\n")
flag_mcmc = "y"
# NOTE: for consistent and good results, choose N > (6/psnr)**2
N = 50  # no. of SRGO data points acquired during T_obs
psnr = 1000.0  # peak signal to noise ratio
# "ligo" (Whittle likelihood in Fourier space) or "custom" likelihood functions for MCMC
likelihood_type = "ligo"
flag_angles = "y"  # input("Plot time-evolution of Euler angles? ([y]/n)\n")
# ---------------------------------------------------------------------------------------------------------
...
r = (
    0.32 * au
)  # initial separation between bodies in au (such that fGW_initial = 24*f_Earth)
# luminosity distance of the binary system in parsec
d = 987.7 * (10.0 ** 6) * pc
Z = 0.2  # redshift of the binary system
inc = inc  # inclination angle of observer w.r.t binary system in degrees


# GW waveform:
""" Dominant harmonic of linearly polarised GWs from non-spinning binary inspirals.
 Inspiral phase of non-spinning binary system can be modelled using post-Newtonian analysis.
...
phase = phase  # GW signal initial phase in degrees.
...
def h_plus(t, inc, phase):
    ...
    return (
        4.0 / d
        * (G * M / c ** 2) ** (5.0 / 3.0)
        * ((pi * f(t) / c) ** (2.0 / 3.0)).astype(floatX)
        * ((1.0 + cos(inc) ** 2.0) / 2.0).astype(floatX)
        * cos(2 * pi * f(t) * t + phase).astype(floatX)
    ).astype(floatX)


def h_cross(t, inc, phase):
    ...
    return (
        ...
        * sin(2 * pi * f(t) * t + phase).astype(floatX)
    ).astype(floatX)


# Storage ring parameters and timing quantities:
v0 = 0.70 * c  # SRGO ion bunch speed
L = 100.0  # SRGO circumference in meters
n_p = 1  # SRGO number of bunches
T_sample = (L / v0) * (1.0 / n_p)  # SRGO bunch clock least count
T_obs = T_obs  # duration of SRGO observation run
N = N  # array size (no. of data points)
...
def time(n, t_obs):
    ...
    t = np.arange(0.0, t_obs, T_timing, dtype=floatX)  # timing data points
    return t, T_timing
...
ra = (
    (RA[0] * 3600.0 + RA[1] * 60.0 + RA[2]) * we / radian
...
lst = (LST[0] * 3600.0 + LST[1] * 60.0 + LST[2]) * we / radian
...
def h_eff(t, phi, theta, psi, inc, phase):
    """Effective longitudinal GW strain for SRGO test masses
    that are circulating rapidly compared to the GW frequency."""
    return (
        -(1.0 / 2.0)
        * (
            h_plus(t, inc, phase) * F_plus(phi, theta, psi)
            + h_cross(t, inc, phase) * F_cross(phi, theta, psi)
        )
    )


def boole_quad(y, x, yofx, a, b, c, d, e):
    ...
    return (2.0 * h / 45.0) * (
        7.0 * y[0]
        + 32.0 * yofx(x[0] + h, a1, b1, c1, d, e)
        + 12.0 * yofx(x[0] + 2.0 * h, a2, b2, c2, d, e)
        + 32.0 * yofx(x[0] + 3.0 * h, a3, b3, c3, d, e)
        + 7.0 * y[1]
    )


def SRGO_signal(t, phi, theta, psi, inc, phase):
    """SRGO signal from response to GWs."""
    integrand = h_eff(t, phi, theta, psi, inc, phase)
...
t_plt, dt_plt = time(3000, 3.0*day)
phi_plt, theta_plt, psi_plt = earth_rot(t_plt, ra, lst, dec, psi_eq, "regular")
signal_plt = SRGO_signal(t_plt, phi_plt, theta_plt, psi_plt, inc, phase)
...
if flag_mcmc == "y":
    ...
    psi = psi.astype(floatX)

    signal = SRGO_signal(t, phi, theta, psi, inc, phase).astype(floatX)
    # adding only stochastic noise
    noise = np.random.normal(0.0, scale=max(
        abs(signal_plt)) / psnr, size=N).astype(floatX)
    noisy_signal = (signal + noise).astype(floatX)

    # Applying bandpass filter based on sensitivity curve:
    noisy_signal_f = fft(noisy_signal)
    freqs = fftfreq(N, dt)
    noisy_signal_f = where(abs(freqs) > 0.0001, noisy_signal_f, 0.0)
    noisy_signal_f = where(abs(freqs) < 0.004, noisy_signal_f, 0.0)
    real_nsf = real(noisy_signal_f)
    imag_nsf = imag(noisy_signal_f)
    ...
    def Whittle(model, data, sigma, df):
        ...
        likelihood = (1.0/(2.0*pi*sigma)) * \
            exp**(-2.0*df*abs(model-data)**2.0/sigma)

        return likelihood

    def dft_custom(model, real_data, imag_data, dt):
        """Custom Direct Fourier Transform function compatible with Theano and PyMC3"""

        N = len(real_data)

        real_model = array([sum([model[n]*cos(2.0*pi*k*n/N)
                           for n in range(N)]) for k in range(N)])*dt
        imag_model = array([sum([model[n]*sin(2.0*pi*k*n/N)
                                 for n in range(N)]) for k in range(N)])*dt

        abs_fdata_minus_fmodel = (
            (real_model-real_data)**2.0 + (imag_model-imag_data)**2.0
        )**0.5
        return abs_fdata_minus_fmodel
    ...
    with MCMC:
        ...
        ra_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "GW source RA (in degrees) posterior", lower=ra-30.0, upper=ra+30.0
        )
        dec_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=-90.0, upper=90.0)(
            "GW source DEC (in degrees) posterior", lower=dec-30.0, upper=dec+30.0
        )
        psi_eq_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=180.0)(
            "GW polarization (in degrees) posterior", lower=psi_eq-30.0, upper=psi_eq+30.0
        )
        phase_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "GW initial phase (in degrees) posterior", lower=phase-30.0, upper=phase+30.0
        ...
        if likelihood_type == "custom":
            ...
            model = abs(sum(
                noisy_signal -
                SRGO_signal(t, phim, thetam, psim, inc, phase_prior)
            )) / denominator
            model = (
            ...
            model = SRGO_signal(t, phim, thetam, psim, inc, phase_prior)
            ...
            model = dft_custom(model, real_nsf, imag_nsf, dt)
    ...
    if __name__ == "__main__":  # needed for MCMC to use multiple CPU cores
        with MCMC:
            ...
            ax_az = az.plot_pair(
                posterior,
                var_names=[
                    "GW source RA (in degrees) posterior",
                    "GW source DEC (in degrees) posterior",
                ],
                kind="kde",
                marginals=True,
                figsize=(9.25, 9.25),
                kde_kwargs={
                    # Plot 90%, 80%, 70% HDI contours (highest density credibility regions)
                    # (x% credible region => (100 - x)% argument value)
                    "hdi_probs": [0.10, 0.20, 0.30],
                    "contour": True,
                    "contourf_kwargs": {"cmap": "magma"},
                    "fill_last": False,
                    "label": "90%, 80%, 70% HDI contours (highest density credibility regions)",
                    "legend": True,
                },
            )
            ...
            axm.scatter(ras, decs, c=imgm, marker='.', s=0.07, linewidths=0.0,
                        cmap='magma', zorder=99,
                        # norm=mpl_colors.Normalize(vmin=0.0, vmax=1.0), alpha=1.0,
                        label="90%, 80%, 70% HDI contours\n(highest density credibility regions)")
            ...
            axm.set_xlabel(r'$\mathrm{RA [hrs]}$')
            axm.xaxis.label.set_fontsize(12)
            axm.set_ylabel(r'$\mathrm{DEC[\degree]}$')
...
if flag_mcmc == "y":
    ...
    m_p = 1.0
    m_n = 1.0
ax.axvline(x=day / 3600, linewidth=0.95, color="orange", label="Sidereal day")
...
if flag_angles == "y":
    ...
    ax1.set_ylabel("Angle in degrees", fontweight="bold")
