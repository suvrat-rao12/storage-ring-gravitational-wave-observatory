...
psnr = 5  # peak signal to noise ratio
...
# plot normalized sky-localization precision vs. GW source position ([y]/n)
flag_srcpos_skyloc = "n"
# ---------------------------------------------------------------------------------------------------------
...
def f(t, f0, k, Z):
    ...
    if flag_mcmc == "y" and flag_sensi != "y" and flag_err != "y" and flag_range != "y" and flag_srcpos_skyloc != "y" and str(type(t)) == "<class 'numpy.ndarray'>":
        return (array([
...
def time(n, t_obs, t_ins, f0, k, Z):
    ...
    if flag_mcmc == "y" and flag_sensi != "y" and flag_err != "y" and flag_range != "y" and flag_srcpos_skyloc != "y":
    ...
    if flag_mcmc == "y" and flag_sensi != "y" and flag_err != "y" and flag_range != "y" and flag_srcpos_skyloc != "y" and len(t) > n:
        # to be able to use FFT trick for computing speed
        pts = len(t) - n
        t = t[:-pts]
    return t, T_timing
...
if flag_srcpos_skyloc == "y":
    ...
    H = array([[0.7293, 3.156375, 6.4074, 0.5202, 2.7176, 1.36815, 39.03733333, 22.4999, 0.7293, 3.156375, 6.4074, 0.5202, 2.7176, 1.36815, 39.03733333, 22.4999, 0.7293],
               [40.4946, 11.19235, 4.5749, 11.9112, 519.809, 64.1406, 0.1656, 15.838575, 17.4665,
                   3.368566667, 10.242975, 33.40513333, 20.45005, 4.5654, 1.33705, 16.71365, 40.4946],
               [34.9914, 11.0959, 4.4157, 23.6215, 16.3359, 447.959, 12.48385, 187.328175, 50.727625,
                   1.5517, 19.6222, 78.9697, 14.004875, 38.8692, 298.8511, 17.3273, 34.9914],
               [11.215525, 294.6978333, 1.0839, 72.68026667, 250.8835, 100., 5.9713, 8.501425,
                   29.80235, 20.87676667, 12.3894, 24.382525, 20.687, 79.87, 180.1059, 27.98315, 11.215525],
               [41.0442, 18.7826, 174.4691333, 65.906925, 31.67793333, 76.14665, 161.8483333, 13.26976667, 41.0442,
                   18.7826, 174.4691333, 65.906925, 31.67793333, 76.14665, 161.8483333, 13.26976667, 41.0442],
               [29.80235, 20.87676667, 12.3894, 24.382525, 20.687, 78.87045, 180.1059, 27.98315,
                   11.215525, 294.6978333, 1.0839, 72.68026667, 250.8835, 100., 5.9713, 8.501425, 29.80235],
               [50.727625, 1.5517, 19.6222, 78.9697, 14.004875, 38.8692, 298.8511, 17.3273,
                   34.9914, 11.0959, 4.4157, 23.6215, 16.3359, 447.959, 12.48385, 187.328175, 50.727625],
               [17.4665, 3.368566667, 10.242975, 33.40513333, 20.45005, 4.5654, 1.33705, 16.71365,
                   40.4946, 11.19235, 4.5749, 11.9112, 519.809, 64.1406, 0.1656, 15.838575, 17.4665],
               [0.7293, 3.156375, 6.4074, 0.5202, 2.7176, 1.36815, 39.03733333, 22.4999, 0.7293,
                   3.156375, 6.4074, 0.5202, 2.7176, 1.36815, 39.03733333, 22.4999, 0.7293],
               ])
    ...
    ras = np.linspace(-180.0, 180.0, 6*360)
    decs = np.linspace(90.0, -90.0, 6*180)
    H = H_func(ras, decs)

    H = np.fliplr(H)
    H = 1.0 - H/(180.0*360.0)  # area to precision conversion
    ...
    figm.tight_layout()
    figm.tight_layout()
    figm.savefig("saved_plots/skyloc_vs_srcpos.png", format="png", dpi=1200)

# Creating synthetic signal and adding only stochastic noise:
if flag_sensi != "y" and flag_err != "y" and flag_range != "y" and flag_srcpos_skyloc != "y":
