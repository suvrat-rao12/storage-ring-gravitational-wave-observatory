...
def f(t, f0):
    ...
    if flag_mcmc == 'y' and str(type(t)) == "<class 'numpy.ndarray'>":
        return (array([
...
def h_eff(t, phi, theta, psi, inc, phase, d, f0):
    ...
    return (
        -(1.0 / 2.0)
        * (
            F_plus(phi, theta, psi) * h_plus(t, inc, phase, d, f0)
            + F_cross(phi, theta, psi) * h_cross(t, inc, phase, d, f0)
        )
    )
...
def sensi_curve(d):
    ...
    # global d
    d = d*val_lhs/val_rhs
    return(f(0.0, f0), abs(h_plus(0.0, inc, phase, d, f0)))
...
if flag_mcmc == "y":
    ...
    with MCMC:
        ...
        #    "GW source mass #1 posterior [$kg$]", lower = 0.9*m1, upper = 1.1*m1
        # )
        m2_prior = m2  # pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
        #    "GW source mass #2 posterior [$kg$]", lower=0.9*m2, upper=1.1*m2
        # )
        r_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "Initial binary separation posterior [$m$]", lower=0.9*r, upper=1.1*r
        )
        ...
        d_prior = pm.Uniform(  # pm.Bound(pm.Normal, lower=0.0, upper=360.0)(
            "GW source distance posterior [$m$]", lower=0.9*d, upper=1.1*d
        )
        ...
        # Model:
        M = (1 + Z) * ((m1_prior * m2_prior) ** (3.0 / 5.0)) / \
            ((m1_prior + m2_prior) ** (1.0 / 5.0))  # chirp mass
        # max(m1,m2) without using boolean operations for Theano compatibility
        max_m1_m2 = (((m1_prior - m2_prior)**2.0) **
                     0.5 + m1_prior + m2_prior)/2.0
        # final separation between bodies @ ISCO
        Rf = 6.0 * G * max_m1_m2 / c ** 2.0
        f0 = sqrt(G * (m1_prior + m2_prior)) / (2.0 * pi * r_prior **
                                                (3.0 / 2.0))  # initial GW frequency
        f1 = sqrt(G * (m1_prior + m2_prior)) / (
            2.0 * pi * Rf ** (3.0 / 2.0)
        )  # final GW frequency at the end of inspiral phase
        k = (96.0 / 5.0) * (pi ** (8.0 / 3.0)) * \
            ((G * M * (1 + Z)**-1 / c ** 3.0) ** (5.0 / 3.0))
        ...
        if likelihood_type == "custom":
            ...
            model = SRGO_signal(t, phim, thetam, psim,
                                inc_prior, phase_prior, d_prior, f0)
