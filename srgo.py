...
r = 0.5  # initial separation between bodies in au
...
if flag_range == "y":
    ra_vals = np.linspace(-45.0, 90.0, 10)
    dec_vals = np.linspace(-45.0, 90.0, 10)
    psi_eq_vals = np.linspace(-45.0, 90.0, 10)
    ...
    d_array = concatenate((np.logspace(
        np.log10(10*pc), np.log10(d_array1[-1]), l), array([pc*10**4])), axis=None)
    z_array = z_of_d(d_array)
    sig_array = np.zeros(l+1)
    ...
    x0 = d_array[-1]/pc
    y0 = sig_array[-1]
    # final element is only for text-position purpose
    d_arrayp = delete(d_array, -1)
    z_arrayp = delete(z_array, -1)
    sig_arrayp = delete(sig_array, -1)
    axes3.plot(d_arrayp/pc, sig_arrayp/(10**-16),
               linewidth=1.6, color="orange")
    axes3.text(x0, y0/(10**-16),
               "$\sigma_{noise} = 0.1fs$", size=2*txtsize/3, color=textcolour)
    axes3.plot(d_arrayp/pc, sig_arrayp/(10**-15),
               linewidth=1.6, color="orange")
    axes3.text(x0, 0.025*y0/(10**-15),
               "$\sigma_{noise} = 1fs$", size=2*txtsize/3, color=textcolour)
    axes3.plot(d_arrayp/pc, sig_arrayp/(10**-12),
               linewidth=1.6, color="orange")
    axes3.text(x0, y0/(10**-12),
               "$\sigma_{noise} = 1ps$", size=2*txtsize/3, color=textcolour)
    axes3.plot(d_arrayp/pc, sig_arrayp/(10**-9), linewidth=1.6, color="orange")
    axes3.text(x0, y0/(10**-9),
               "$\sigma_{noise} = 1ns$", size=2*txtsize/3, color=textcolour)
    ...
    axes3.set_yticks(concatenate((10.**arange(-1+int(np.log10(min(sig_array)/(10**-9))), -1, 2),
                     array([10.**-1, 10.**0]), 10.**arange(1, 1+int(np.log10(max(sig_array)/(10**-16))), 2)), axis=None))
    ...
    axes4.plot(z_arrayp, sig_arrayp, color='orange', linewidth=0.0)
...
if flag_sensi == "y":
    M1_vals = np.logspace(1.0, 7.0, 1)
    # M2_vals = np.logspace(1.0, 6.0, 6)
    ra_vals = np.linspace(-45.0, 90.0, 7)
    dec_vals = np.linspace(-45.0, 90.0, 7)
    psi_eq_vals = np.linspace(-45.0, 90.0, 7)
