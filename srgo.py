    ...
    if __name__ == "__main__":  # needed for MCMC to use multiple CPU cores
        ...
        with MCMC:
            ...
            posterior = pm.sample(
                ...
                cores=1,
                # start={'m1_prior': m1},
                tune=100,  # burn-in samples / discarded tuning samples
...
if flag_srcpos_skyloc == "y":
    ...
    for j in range(len(decs2)):
        for i in range(len(ras)):
            ...
            H_sig[j].append(peak_sig/(2.358462672796294e-15))
            print(i)
    ...
    bkg = axm2.pcolormesh(ras*radian, decs*radian, H_sig,
                          cmap='magma', alpha=1.0, zorder=97)
    figm2.colorbar(bkg, orientation='vertical')
    axm2.scatter(array([0.0*radian]), array([51.0*radian]), s=50.0, color='springgreen',
                 marker='x', alpha=0.9, zorder=100, label="SRGO initial position")
    ...
    axm2.plot(-np.linspace(180.0*radian, -180.0*radian, 360), 51.0*radian*ones(360), color='springgreen',
              linestyle='dashdot', alpha=0.9, zorder=99, label="SRGO track in 24.0 hrs")
    axm2.plot(
        [], [], ' ', label="Max. value = %r\nPSNR at center = 1.0\nMin. value = %r" % (np.round(np.max(H_sig), 2), np.round(np.min(H_sig), 2)))
    axm2.set_xticklabels(tick_labels_x)
    axm2.set_xlabel("RA [hrs]")
    axm2.xaxis.label.set_fontsize(12)
    axm2.set_ylabel("DEC [$\degree$]")
    ...
    figm2.tight_layout()
    figm2.tight_layout()
    figm2.savefig("saved_plots/psnr_vs_srcpos.png", format="png", dpi=800)
...
if flag_tobs == "y":
    ...
    data = [[], [], []]
    dirloc = ["D:/SRGO T_obs/psnr_0.1",
              "D:/SRGO T_obs/psnr_1", "D:/SRGO T_obs/psnr_10"]
    for j in range(3):
        count = 0
        temp = []
        for i in range(40):
    ...
    plt.scatter(tobs, means, marker='o',
                label="PSNR = 0.1", linewidth=1.5, color='black')
    ...
    plt.scatter(tobs, means3, marker='s',
                label="PSNR = 10", linewidth=1.5, color='gold')
...
if flag_dist == "y":
    ...
    devs = [(cosmo.luminosity_distance(Z+np.mean(x)+np.std(x)).value - cosmo.luminosity_distance(Z+np.mean(x)).value + cosmo.luminosity_distance(Z -
             np.mean(x)).value - cosmo.luminosity_distance(Z-np.mean(x)-np.std(x)).value)*100/(2*cosmo.luminosity_distance(Z).value) for x in data[0]]
    ...
    devs2 = [(cosmo.luminosity_distance(Z+np.mean(x)+np.std(x)).value - cosmo.luminosity_distance(Z+np.mean(x)).value + cosmo.luminosity_distance(Z -
             np.mean(x)).value - cosmo.luminosity_distance(Z-np.mean(x)-np.std(x)).value)*100/(2*cosmo.luminosity_distance(Z).value) for x in data[1]]
    ...
    devs3 = [(cosmo.luminosity_distance(Z+np.mean(x)+np.std(x)).value - cosmo.luminosity_distance(Z+np.mean(x)).value + cosmo.luminosity_distance(Z -
             np.mean(x)).value - cosmo.luminosity_distance(Z-np.mean(x)-np.std(x)).value)*100/(2*cosmo.luminosity_distance(Z).value) for x in data[2]]
