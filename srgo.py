...
if flag_srcpos_skyloc == "y":
    ...
    t_h, dt_h = time(2000, day, t_ins, f0, k, Z)
    H_sig = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], []]
    ...
    #         linestyle='dashdot', alpha=0.9, zorder=99, label="SRGO track in 3.0 h")
    # axm.plot(-np.linspace(180.0*radian, -180.0*radian, 360), 51.0*radian*ones(360), color='red',
    #         linestyle='dashdot', alpha=0.9, zorder=99, label="SRGO track in 24.0 h")
    # axm.plot(
    ...
    #          linestyle='dashdot', alpha=0.9, zorder=99, label="SRGO track in 12.0 h")
    axm2.plot(-np.linspace(180.0*radian, -180.0*radian, 360), (theta_lat-2.*pi)*ones(360), color='springgreen',
              linestyle='dashdot', alpha=0.9, zorder=99, label="SRGO zenith's track in 24.0 h")
    axm2.plot(
    ...
    axm2.set_xlabel("INITIAL HOUR ANGLE [h]")
...
# Sky localization area vs. PSNR & vs. T_obs plots:
if flag_sky == "y":
    ...
    # multiply by 2 to account for the antipodal region.

    means = [min(2*np.mean(x), 41253.) for x in data[0]]
    devs = [min(2*np.std(x), 41253.) for x in data[0]]

    means2 = [min(2*np.mean(x), 41253.) for x in data[1]]
    devs2 = [min(2*np.std(x), 41253.) for x in data[1]]

    means3 = [min(2*np.mean(x), 41253.) for x in data[2]]
    devs3 = [min(2*np.std(x), 41253.) for x in data[2]]

    psnr = np.array([100., 30., 10., 3., 1., 0.3, 0.1])

    esnr_100 = 100*np.array([16, 32, 64])**0.5
    esnr_10 = 10*np.array([16, 32, 64])**0.5
    esnr_1 = 1*np.array([16, 32, 64])**0.5
    sky_100 = np.array([means[0], means2[0], means3[0]])
    sky_10 = np.array([means[2], means2[2], means3[2]])
    sky_1 = np.array([means[4], means2[4], means3[4]])
    skyint_100 = interp1d(esnr_100, sky_100, kind="slinear",
                          fill_value="extrapolate")
    skyint_10 = interp1d(esnr_10, sky_10, kind="slinear",
                         fill_value="extrapolate")
    skyint_1 = interp1d(esnr_1, sky_1, kind="slinear",
                        fill_value="extrapolate")
    ...
    plt.plot(tobs2, skyint_1(1*(2.667*tobs2)**0.5),
    ...
    plt.plot(tobs2, skyint_10(10*(2.667*tobs2)**0.5),
             linestyle='dotted', color='blue')
    ...
    eb3[-1][0].set_linestyle('solid')
    plt.plot(tobs2, skyint_100(100*(2.667*tobs2)**0.5),
             linestyle='dotted', color='blue', label="expected due to $\sqrt{f_{sample}T_{obs}}$\n($f_{sample}=2.666$ data points per hour)")
    highlight_circle = plt.Circle(
        (tobs[2], means3[2]), radius=0.1, color='blue', fill=False)
    plt.gca().add_patch(highlight_circle)
    plt.xlim(0.0, 27.0)
    plt.ylim(5.0, 10.**6)
    plt.xlabel(
        "Observation time [h]")
...
# Relative error of mass estimation vs. PSNR & vs. T_obs plot:
if flag_mass == "y":
    ...
    esnr_10 = 10*np.array([16, 32, 64])**0.5
    esnr_1 = 1*np.array([16, 32, 64])**0.5
    mass_100 = np.array([means[0], means2[0], means3[0]])
    mass_10 = np.array([means[2], means2[2], means3[2]])
    mass_1 = np.array([means[4], means2[4], means3[4]])
    massint_100 = interp1d(esnr_100, mass_100, kind="slinear",
                           fill_value="extrapolate")
    massint_10 = interp1d(esnr_10, mass_10, kind="slinear",
                          fill_value="extrapolate")
    massint_1 = interp1d(esnr_1, mass_1, kind="slinear",
                         fill_value="extrapolate")
    ...
    eb1 = plt.errorbar(tobs, means, yerr=devs, alpha=0.6,
                       linewidth=1.5, color='orange', zorder=2)
    eb1[-1][0].set_linestyle('dashed')
    plt.plot(tobs2, massint_1(1*(2.667*tobs2)**0.5),
             linestyle='dotted', color='blue')
    plt.scatter(tobs, means2, marker='v',
    ...
    eb2[-1][0].set_linestyle('dotted')
    plt.plot(tobs2, massint_10(10*(2.667*tobs2)**0.5),
             linestyle='dotted', color='blue')
    plt.scatter(tobs, means3, marker='s',
    ...
    eb3[-1][0].set_linestyle('solid')
    plt.plot(tobs2, massint_100(100*(2.667*tobs2)**0.5),
             linestyle='dotted', color='blue', label="expected due to $\sqrt{f_{sample}T_{obs}}$\n($f_{sample}=2.666$ data points per hour)")
    highlight_circle = plt.Circle(
        (tobs[2], means3[2]), radius=0.1, color='blue', fill=False)
    plt.gca().add_patch(highlight_circle)
...
# Relative error of distance estimation vs. PSNR & vs. T_obs plot:
if flag_dist == "y":
    ...
    esnr_10 = 10*np.array([16, 32, 64])**0.5
    esnr_1 = 1*np.array([16, 32, 64])**0.5
    dist_100 = np.array([means[0], means2[0], means3[0]])
    dist_10 = np.array([means[2], means2[2], means3[2]])
    dist_1 = np.array([means[4], means2[4], means3[4]])
    distint_100 = interp1d(esnr_100, dist_100, kind="slinear",
                           fill_value="extrapolate")
    distint_10 = interp1d(esnr_10, dist_10, kind="slinear",
                          fill_value="extrapolate")
    distint_1 = interp1d(esnr_1, dist_1, kind="slinear",
                         fill_value="extrapolate")
    ...
    plt.plot(tobs2, distint_1(1*(2.667*tobs2)**0.5),
             linestyle='dotted', color='blue')
    plt.scatter(tobs, means2, marker='v',
    ...
    eb2[-1][0].set_linestyle('dashed')
    plt.plot(tobs2, distint_10(10*(2.667*tobs2)**0.5),
             linestyle='dotted', color='blue')
    plt.scatter(tobs, means3, marker='s',
    ...
    plt.plot(tobs2, distint_100(100*(2.667*tobs2)**0.5),
             linestyle='dotted', color='blue', label="expected due to $\sqrt{f_{sample}T_{obs}}$\n($f_{sample}=2.666$ data points per hour)")
    highlight_circle = plt.Circle(
        (tobs[2], means3[2]), radius=1.0, color='blue', fill=False)
    plt.gca().add_patch(highlight_circle)
    plt.xlim(0.0, 27.0)
    plt.ylim(10.1, 1000.0)
    plt.xlabel(
        "Observation time [h]")
    plt.ylabel("Relative error of distance estimation [%]")
    plt.minorticks_on()
...
if flag_sensi != "y" and flag_err != "y" and flag_range != "y" and flag_srcpos_skyloc != "y" and flag_sky != "y" and flag_mass != "y" and flag_dist != "y" and flag_frq != "y":
    ...
    ax.text(
        ...
        % (
            ...
            np.round(f(0, f0, k, Z), decimals=5),
    ...
    formatter = mpl_ticker.ScalarFormatter(
        useMathText=True, useOffset=False, useLocale=False)
    ...
    if flag_extras == "y":
        ...
        # Signal spectrogram:
        # https://docs.scipy.org/doc//scipy-1.3.3/reference/generated/scipy.signal.spectrogram.html
        # https://in.mathworks.com/help/signal/ref/spectrogram.htmlhttps://in.mathworks.com/help/signal/ref/spectrogram.html
        # https://dsp.stackexchange.com/questions/248/how-do-i-optimize-the-window-lengths-in-stft
        # https://dsp.stackexchange.com/questions/16897/improve-spectrogram-appearance-using-windowing-functions-matlab
        # https://en.wikipedia.org/wiki/Spectrogram
        fig_sg, ax_sg = plt.subplots(1, 1)
        f_sg, t_sg, Sxx = spectrogram(
            signal_plt, fs=1.0/dt_plt, window='hanning', nperseg=int(1.75/(f(t[-1], f0, k, Z)*dt_plt)), scaling='density')
        im = ax_sg.pcolormesh(t_sg / 3600.0, f_sg, Sxx,
                              shading='gouraud', cmap='viridis')
        cbar = fig_sg.colorbar(im, ax=ax_sg)
        cbar.set_label("[$s^2$/Hz]", fontsize=16)
        cbar.formatter = formatter
        cbar.update_ticks()
        ax_sg.set_ylabel("Frequency [Hz]", fontsize=16)
        ax_sg.set_xlabel("Time [h]", fontsize=16)  # \n1 unit = %r h" % tick)
        ...
        # \n1 unit = %r h" %tick, fontweight="bold")
        # , fontweight="bold")
        ax1.set_ylabel("Angle [$\degree$]", fontsize=16)
