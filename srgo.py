...
Z = 0.5  # redshift of the binary system
...
if flag_srcpos_skyloc == "y":
    ...
    axm2.plot(
        [], [], ' ', label="For 51$\degree$ SRGO latitude, PSNR at center = 1.0\nMax. value = %r\nAverage value = %r\nMin. value = %r" % (np.round(np.max(H_sig), 2), np.round(np.mean(H_sig), 2), np.round(np.min(H_sig), 2)))
